...
void Room::dispatchMessages() {//inutile
...
MessageSymbol Room::eraseSymbol(const std::string& filename, int startIndex,int endIndex, int id){
    ...
    this->room_map_.at(filename).erase(this->room_map_.at(filename).begin()+startIndex,this->room_map_.at(filename).begin()+endIndex);
